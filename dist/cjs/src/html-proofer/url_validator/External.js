...
const Hydra_1 = require("../Hydra");
const Utils_1 = require("../Utils");
const Failure_1 = require("../Failure");
const outdent_1 = require("outdent");
class External extends UrlValidator_1.UrlValidator {
    constructor(runner, external_urls) {
        super(runner);
        this.paths_with_queries = new Map();
        this.external_urls = external_urls;
        this.before_request = [];
        this.hydra = new Hydra_1.Hydra(runner.logger);
    }
    validate() {
    ...
    run_external_link_checker(links) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const [external_url, metadata] of links) {
                const url = new Url_1.Url(this.runner, external_url);
                if (!url.valid()) {
    ...
    handle_connection_failure(href, metadata, response_code, status_message) {
        const msgs = [(0, outdent_1.outdent) `
    ...
    add_failure(metadata, description, status = null) {
        if (metadata.length === 0) { // possible if we're checking an array of links
            this.failed_checks.push(new Failure_1.Failure('', 'Links > External', description, null, status));
        }
        for (const m of metadata) {
            this.failed_checks.push(new Failure_1.Failure(m.filename, 'Links > External', description, m.line, status));
        }
    }
