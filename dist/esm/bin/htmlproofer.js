...
import { VERSION } from '../src/html-proofer/Version';
import { isDirectory } from '../src/html-proofer/Utils';
...
program.
    ...
    if (opts.asLinks) {
        const links = opts.asLinks.split(',')
            .flatMap((e) => e.split(','))
            .map((e) => e.trim());
        yield HTMLProofer.check_links(links, options).run();
    }
    else if (isDirectory(paths[0])) {
        yield HTMLProofer.check_directories(paths, options).run();
    }
    else {
        yield HTMLProofer.check_file(path, options).run();
    }
}));
