...
export class External extends UrlValidator {
    ...
    validate() {
        if (this.cache.enabled()) {
            ...
            this.run_external_link_checker(urls_to_check)
        } else {
            this.run_external_link_checker(this.external_urls)
        }
    ...
    response_handler(response, url, filenames) {
        ...
        response.body = response.body.replace("\x00", "")
    ...
    check_hash_in_2xx_response(href, url, response, filenames) {
        ...
        const body_doc = create_nokogiri(response.body)
        ...
        if (body_doc.xpath(xpath.join("|"))) {
            return
        }

        const msg = "External link #{href} failed: #{url.sans_hash} exists, but the hash '#{hash}' does not"
        this.add_failure(filenames, msg, response.code)
        this.cache.add_external(href, filenames, response.code, msg)
        return true
    }
