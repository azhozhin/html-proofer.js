...
export class Internal extends UrlValidator {
    ...
    run_internal_link_checker(links){
        ...
        for(const [link, matched_files] of Object.entries(links)){
            for (const metadata in matched_files){
                ...
                if (!this.hash_exists(url)){
                    this.failed_checks.push(new Failure(this.runner.current_filename, "Links > Internal",
                        `internally linking to ${url}; the file exists, but the hash '${url.hash}' does not`, metadata['line'], null, null))
                    to_add.push([url, metadata, false])
                    continue
                }
    ...
    file_exists(url){
        ...
        if (this.runner.checked_paths.key(absolute_path)){
    ...
    hash_exists(url){
        ...
        return fragment_ids.includes("top") || (!this.find_fragments(fragment_ids, url))
    }


    find_fragments(fragment_ids, url){
        const xpaths = fragment_ids.uniq.flatMap(frag_id => {
