...
export class Url extends Attribute {
    ...
    non_http_remote() {
        return this.scheme != null && !this.remote
    }

    get host(){
        return this.parts ? this.parts.host : null
    }
    ...
    get query_values(){
        return this.parts ? this.parts.query_values : null
    }
    ...
    base64(){
        this.raw_attribute.match(/^data:image/)
    }
    ...
    get file_path(){
        ...
        // either overwrite with root_dir; or, if source is directory, use that; or, just get the current file's dirname
            base = this.runner.options['root_dir'] || (isDirectory(this.runner.current_source) ? this.runner.current_source : File.dirname(this.runner.current_source))
        //relative links, path is a file
        } else if (fs.existsSync(path.resolve(this.runner.current_source, this.path)) || fs.existsSync(path.resolve(this.runner.current_source, path_dot_ext))){
            base = path.dirname(this.runner.current_filename)
            // relative links in nested dir, path is a file
        } else if (fs.existsSync(File.join(File.dirname(this.runner.current_filename), this.path)) || fs.existsSync(File.join(path.dirname(this.runner.current_filename), path_dot_ext))){
            base = path.dirname(this.runner.current_filename)
