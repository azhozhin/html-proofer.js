...
import {Cli} from './reporter/cli.js'
import {create_nokogiri, mergeConcat, pluralize} from './utils.js'
import {Check} from './check.js'
import {External} from './url_validator/external.js'
import {Internal} from './url_validator/internal.js'
...
export class Runner {
  ...
  async run() {
    ...
    if (this.type === 'links') {
      ...
      this.logger.log('info',
          `Running ${check_text} (${this.format_checks_list(this.checks)}) in ${normalize_path(
              this.source)} on *${this.options['extensions'].join(', ')} files...\n\n`)
  ...
  check_parsed(p, source) {
    ...
    for (const ch of this.checks) {
      ...
      this.logger.log('debug', `Running ${check.short_name} in ${p}`)
  ...
  report_failed_checks() {
    ...
    // todo: process.exit(1) works, but jest is failing
    //process.exitCode = 1
