...
export class OpenGraph extends Check {
    ...
    run() {
        this.html('meta[property="og:url"], meta[property="og:image"]').each((i, node) => {
            ...
            if (this.missing_content()) {
                ...
                if (!this.open_graph.url.exists()) {
                    this.add_failure(`internal open graph ${this.open_graph.url.raw_attribute} does not exist`, this.open_graph.line, null, this.open_graph.content)
                }
            }
        })
