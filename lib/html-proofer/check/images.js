  ...
  run() {
    this.html.css('img').each((i, node) => {
      ...
      if (this.terrible_filename()) {
        this.add_failure(`image has a terrible filename (${this.img.url.raw_attribute})`, this.img.line, null,
            this.img.content)
      }
      ...
      if (this.missing_src()) {
        ...
        this.add_failure(`internal image ${this.img.url.raw_attribute} does not exist`, this.img.line, null,
            this.img.content)
      ...
      if (!this.ignore_element()) {
        if (this.missing_alt_tag() && !this.ignore_missing_alt()) {
          this.add_failure(`image ${this.img.url.raw_attribute} does not have an alt attribute`, this.img.line, null,
              this.img.content)
        } else if ((this.empty_alt_tag() || this.alt_all_spaces()) && !this.ignore_empty_alt()) {
          this.add_failure(`image ${this.img.url.raw_attribute} has an alt attribute, but no content`, this.img.line,
              null, this.img.content)
        }
      }
      if (this.runner.enforce_https() && this.img.url.http()) {
        this.add_failure(`image ${this.img.url.raw_attribute} uses the http scheme`, this.img.line, null,
            this.img.content)
      }
