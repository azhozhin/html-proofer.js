...
export class Links extends Check {
    ...
    this.html.css('a, link, source').each((i, node) => {
      ...
      if (!this.link.url.internal && this.link.url.remote) {
        ...
        if (this.link.url.unslashed_directory(this.link.url.absolute_path)) {
          this.add_failure(`internally linking to a directory ${this.link.url.raw_attribute} without trailing slash`,
  ...
  handle_mailto() {
    if (!this.link.url.path) {
      if (!this.ignore_empty_mailto()) {
        this.add_failure(`${this.link.url.raw_attribute} contains no email address`, this.link.line, null,
            this.link.content)
      }
    } else if (!this.link.url.path.match(this.EMAIL_REGEXP)) {
      this.add_failure(`${this.link.url.raw_attribute} contains an invalid email address`, this.link.line, null,
          this.link.content)
    }
  }
  ...
  handle_tel() {
    if (!this.link.url.path) {
      this.add_failure(`${this.link.url.raw_attribute} contains no phone number`, this.link.line, null,
          this.link.content)
    }
  }
  ...
  check_sri() {
    ...
    if ((!this.link.node['integrity']) && (!this.link.node['crossorigin'])) {
      this.add_failure(`SRI and CORS not provided in: ${this.link.url.raw_attribute}`, this.link.line, null,
          this.link.content)
    } else if (!this.link.node['integrity']) {
      this.add_failure(`Integrity is missing in: ${this.link.url.raw_attribute}`, this.link.line, null,
          this.link.content)
    } else if (!this.link.node['crossorigin']) {
      this.add_failure(`CORS not provided for external resource in: ${this.link.url.raw_attribute}`, this.link.line,
          null, this.link.content)
    }
