...
export class Links extends Check {
    run() {
        this.html("a, link, source").each((i, node) => {
            ...
            if (!this.link.url.internal && this.link.url.remote) {
                ...
                this.add_to_external_urls(this.link.url, thus.link.line)
    ...
    check_schemes() {
        ...
        this.add_failure("#{@link.url.raw_attribute} is not an HTTPS link", this.link.line, null, this.link.content)
    }
