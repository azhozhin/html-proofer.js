    ...
    handle_mailto() {
        if (!this.link.url.path) {
            if (!this.ignore_empty_mailto()) {
                this.add_failure(`${this.link.url.raw_attribute} contains no email address`, this.link.line, null, this.link.content)
            }
        } else if (!this.link.url.path.match(this.EMAIL_REGEXP)) {
            this.add_failure(`${this.link.url.raw_attribute} contains an invalid email address`, this.link.line, null, this.link.content)
        }
    }
