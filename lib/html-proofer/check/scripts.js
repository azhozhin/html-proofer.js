...
export class Scripts extends Check {
    run() {
        this.html.css("script").each((i, node) => {
            ...
            if (!isNullOrEmpty(this.script.content.strip())) {
                return
            }
            ...
            if (this.missing_src()) {
                ...
                this.add_failure("internal script reference #{@script.src} does not exist", this.script.line, null, this.script.content)
            }
    ...
    check_sri() {
        if (isNullOrEmpty(this.script.node["integrity"]) && isNullOrEmpty(this.script.node["crossorigin"])) {
            this.add_failure(`SRI and CORS not provided in: #{@script.url.raw_attribute}`, this.script.line, null, this.script.content)
        } else if (isNullOrEmpty(this.script.node["integrity"])) {
            this.add_failure(`Integrity is missing in: #{@script.url.raw_attribute}`, this.script.line, null, this.script.content)
        } else if (isNullOrEmpty(this.script.node["crossorigin"])) {
            this.add_failure(`CORS not provided for external resource in: #{@script.url.raw_attribute}`, this.script.line, null, this.script.content)
        }
    }
