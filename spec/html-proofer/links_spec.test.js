...
describe('Links test', () => {
  ...
  it('fails for broken internal hash', async () => {
    ...
    expect(proofer.failed_checks.first.description).
        toMatch(/internally linking to #noHash; the file exists, but the hash 'noHash' does not/)
  })
  ...
  it('accepts multiple filenames', async () => {
    ...
    const results = proofer.failed_checks.map(x => x.path).
  ...
  it('fails for broken internal links', async () => {
    ...
    expect(proofer.failed_checks.first.description).
        toMatch('internally linking to ./notreal.html, which does not exist')
  })
  ...
  it('fails for broken internal root links', async () => {
    ...
    expect(proofer.failed_checks.first.description).
        toMatch('internally linking to /broken_root_link_internalz.html, which does not exist')
  })
  ...
  it('fails for broken hash with query', async () => {
    ...
    expect(proofer.failed_checks.first.description).
        toMatch(/#example; the file exists, but the hash 'example' does not/)
  })
  ...
  it('fails for mismatched hash casing', async () => {
    ...
    const proofer = await run_proofer(hash_on_another_page, 'file')
    expect(proofer.failed_checks.first.description).
        toMatch('#MainMenu; the file exists, but the hash \'MainMenu\' does not')
  })
  ...
  it('reports linked internal through directory', async () => {
    ...
    expect(proofer.failed_checks.first.description).
        toMatch(/the file exists, but the hash 'generating-and-submitting' does not/)
  })
  ...
  it('should not try reading PDFs', async () => {
    ...
    expect(proofer.failed_checks.length).toEqual(3)
    expect(proofer.failed_checks.first.description).
        toMatch(/internally linking to exists.pdf#page=2; the file exists, but the hash 'page=2' does not/)
    expect(proofer.failed_checks.last.description).
