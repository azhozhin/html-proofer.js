...
describe('Images test', () => {
  it('passes for existing external images', async () => {
    ...
    const proofer = await run_proofer(external_image_filepath, CheckType.FILE)
  ...
  it('fails for image missing alt attribute', async () => {
    ...
    const proofer = await run_proofer(missing_alt_filepath, CheckType.FILE)
    expect(proofer.failed_checks.length).toEqual(1)
    expect(proofer.failed_checks[0].description).toMatch(/gpl.png does not have an alt attribute/)
  })

  it('does not fail for image missing alt attribute when asked to ignore', async () => {
    const missing_alt_filepath = path.join(FIXTURES_DIR, 'images', 'missing_image_alt.html')
    const opts: IOptions = {ignore_missing_alt: true}
    const proofer = await run_proofer(missing_alt_filepath, CheckType.FILE, opts)
    expect(proofer.failed_checks).toEqual([])
  })
  ...
  it('fails for image with an empty alt attribute when asked', async () => {
    ...
    const proofer = await run_proofer(missing_alt_filepath, CheckType.FILE, opts)
    expect(proofer.failed_checks.length).toEqual(1)
    expect(proofer.failed_checks[0].description).toMatch(/gpl.png has an alt attribute, but no content/)
  })
  ...
  it('does not fail for image with nothing but spaces in alt attribute', async () => {
    ...
    const proofer = await run_proofer(empty_alt_filepath, CheckType.FILE)
    expect(proofer.failed_checks).toEqual([])
  })
  ...
  it('fail for image with nothing but spaces in alt attribute when asked', async () => {
    const empty_alt_filepath = path.join(FIXTURES_DIR, 'images', 'empty_image_alt_text.html')
    const opts: IOptions = {ignore_empty_alt: false}
    const proofer = await run_proofer(empty_alt_filepath, CheckType.FILE, opts)
    expect(proofer.failed_checks.length).toEqual(1)
    expect(proofer.failed_checks[0].description).toMatch(/gpl.png has an alt attribute, but no content/)
  })
  ...
  it('passes for missing internal images even when ignore_urls is set', async () => {
    ...
    const opts: IOptions = {ignore_urls: [/.*/]};
    const proofer = await run_proofer(internal_image_filepath, CheckType.FILE, opts)
    expect(proofer.failed_checks).toEqual([])
  })

  it('passes for images with spaces all over', async () => {
    const spaced_filepath = path.join(FIXTURES_DIR, 'images', 'spaced_image.html')
    const proofer = await run_proofer(spaced_filepath, CheckType.FILE)
    expect(proofer.failed_checks).toEqual([])
  })
  ...
  it('fails for missing internal images', async () => {
    const internal_image_filepath = path.join(FIXTURES_DIR, 'images', 'missing_image_internal.html')
    const proofer = await run_proofer(internal_image_filepath, CheckType.FILE)
    expect(proofer.failed_checks.length).toEqual(1)
  ...
  it('fails for image with no src', async () => {
    ...
    const proofer = await run_proofer(image_src_filepath, CheckType.FILE)
    expect(proofer.failed_checks.length).toEqual(1)
    expect(proofer.failed_checks[0].description).toMatch(/image has no src or srcset attribute/)
  })
  ...
  it('fails for image with default macOS filename', async () => {
    ...
    const proofer = await run_proofer(terrible_image_name, CheckType.FILE)
    expect(proofer.failed_checks.length).toBeGreaterThanOrEqual(1)
    expect(proofer.failed_checks[0].description).toMatch(/image has a terrible filename/)
  })
  ...
  it('ignores images marked as ignore data-proofer-ignore', async () => {
    ...
    const proofer = await run_proofer(ignorable_images, CheckType.FILE)
    expect(proofer.failed_checks).toEqual([])
  })
  ...
  it('ignores images via ignore_urls', async () => {
    ...
    const proofer = await run_proofer(ignorable_image, CheckType.FILE, opts)
    expect(proofer.failed_checks).toEqual([])
  })
  ...
  it('properly ignores missing alt tags when asked', async () => {
    ...
    const proofer = await run_proofer(ignoreable_links, CheckType.FILE, opts)
    expect(proofer.failed_checks).toEqual([])
  })
  ...
  it('works for images with a srcset', async () => {
    ...
    const proofer = await run_proofer(src_set_check, CheckType.FILE)
  ...
  it('fails for images with a srcset but missing alt', async () => {
    ...
    const proofer = await run_proofer(src_set_missing_alt, CheckType.FILE)
    expect(proofer.failed_checks.length).toBeGreaterThanOrEqual(1)
    expect(proofer.failed_checks[0].description).toMatch(/image gpl.png does not have an alt attribute/)
  })
  ...
  it('translates src via swap_urls', async () => {
    ...
    const proofer = await run_proofer(translate_src, CheckType.FILE, opts)
    expect(proofer.failed_checks).toEqual([])
  })
  ...
  it('passes for HTTP images when asked', async () => {
    ...
    const proofer = await run_proofer(http, CheckType.FILE, opts)
    expect(proofer.failed_checks).toEqual([])
  })

  it('fails for HTTP images when not asked', async () => {
    const http = path.join(FIXTURES_DIR, 'images', 'src_http.html')
    const proofer = await run_proofer(http, CheckType.FILE)
    expect(proofer.failed_checks.length).toEqual(1)
    expect(proofer.failed_checks[0].description).toMatch(/uses the http scheme/)
  })
  ...
  it('properly checks relative images with base', async () => {
    ...
    const proofer = await run_proofer(relative_images, CheckType.FILE)
    expect(proofer.failed_checks).toEqual([])
  })
  ...
  it('supports multiple srcsets when fails', async () => {
    ...
    const proofer = await run_proofer(relative_images, CheckType.FILE)
    expect(proofer.failed_checks.length).toEqual(1)
    expect(proofer.failed_checks[0].description).toEqual('internal image /uploads/150-marie-lloyd.jpg 1.5x does not exist')
  })
  ...
  it('works for images with a swapped data attribute src', async () => {
    ...
    const proofer = await run_proofer(custom_data_src_check, CheckType.FILE, opts)
    expect(proofer.failed_checks).toEqual([])
  })
