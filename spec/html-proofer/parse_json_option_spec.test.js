...
describe('JSON config parser', () => {
  ...
  it('Throws an error when the option name is not a string', () => {
    expect(() => {
      Configuration.parse_json_option(123, '')
  ...
  it('Throws an error when the json config is not a string', () => {
    expect(() => {
      Configuration.parse_json_option('testName', 123)
    }).toThrowError(new Error('ArgumentError: Must provide a JSON configuration in string format.'))
  })
