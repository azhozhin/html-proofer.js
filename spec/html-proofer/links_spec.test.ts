...
describe('Links test', () => {
  it('fails for broken internal hash (even if the file exists)', async () => {
    ...
    const proofer = await run_proofer(broken_hash_external_filepath, CheckType.FILE)
    expect(last(proofer.failed_checks).description).toMatch(
  ...
  it('fails for broken hashes on the web when asked (even if the file exists)', async () => {
    ...
    const proofer = await run_proofer(broken_hash_on_the_web, CheckType.FILE)
    expect(proofer.failed_checks[0].description).toMatch(/but the hash 'no' does not/)
  })

  it('passes for broken hashes on the web when ignored (even if the file exists)', async () => {
    const broken_hash_on_the_web = path.join(FIXTURES_DIR, 'links', 'broken_hash_on_the_web.html')
    const proofer = await run_proofer(broken_hash_on_the_web, CheckType.FILE, {check_external_hash: false})
  ...
  it('succeeds for working root-link with hash', async () => {
    ...
    const proofer = await run_proofer(file, CheckType.FILE, {root_dir: 'spec/html-proofer/fixtures/links/root_folder'})
    expect(proofer.failed_checks).toEqual([])
  })
  ...
  it('translates links via swap_urls for list of links', async () => {
    ...
    const proofer = await run_proofer(['http://www.garbalarba.com'], CheckType.LINKS, opts)
    expect(proofer.failed_checks).toEqual([])
  })
  ...
  it('allows for internal linking to a directory without trailing slash by default', async () => {
    ...
    const proofer = await run_proofer(internal, CheckType.FILE)
    expect(proofer.failed_checks).toEqual([])
  })
  ...
  it('fails for internal linking to a directory without trailing slash', async () => {
    const options = {typhoeus: {followlocation: false}}
    const internal = path.join(FIXTURES_DIR, 'links', 'link_directory_without_slash.html')
    const proofer = await run_proofer(internal, CheckType.FILE, options)
    expect(proofer.failed_checks[0].description).toMatch(/without trailing slash/)
  })
  ...
  it('ignores external links when asked', async () => {
    ...
    const proofer = await run_proofer(external, CheckType.FILE, options)
    expect(proofer.failed_checks).toEqual([])
  })
  ...
  it('works for array of links', async () => {
    const proofer = await run_proofer(['www.github.com', 'foofoofoo.biz'], CheckType.LINKS)
    expect(proofer.failed_checks[0].description).toMatch(/failed with something very wrong/)
  })
  ...
  describe('automatically adding default extensions to files', () => {
    ...
    it('can be turned off', async () => {
      ...
      const proofer = await run_proofer(context.fixture, CheckType.FILE, {assume_extension: ''})
      expect(proofer.failed_checks.length).toBeGreaterThanOrEqual(3)
    })

    it('accepts extensionless file links by default', async () => {
      // With command-line option
      const proofer = await run_proofer(context.fixture, CheckType.FILE)
      expect(proofer.failed_checks).toEqual([])
    })
  })
  ...
  it('does not explode on bad external links in files', async () => {
    ...
    expect(proofer.failed_checks[0].description).toMatch(/is an invalid URL/)
  })

  it('does not explode on bad external links in arrays', async () => {
    const proofer = await run_proofer(['www.github.com', 'http://127.0.0.1:____'], CheckType.LINKS)
    expect(proofer.failed_checks[0].description).toMatch(/is an invalid URL/)
  })
  ...
  it('does not bomb on dns-prefetch', async () => {
    ...
    const proofer = await run_proofer(prefetch, CheckType.FILE)
    expect(proofer.failed_checks).toEqual([])
  })
  ...
  it('handles timeout', async () => {
    const proofer = await run_proofer(['https://www.sdskafnlkn3rl3204uasfilfkmakmefalkm.com:81'], CheckType.LINKS)
    expect(proofer.failed_checks[0].description).toMatch(/got a time out|the request timed out/)
  })

  it('correctly handles empty href', async () => {
    const file = path.join(FIXTURES_DIR, 'links', 'empty_href.html')
    const proofer = await run_proofer(file, CheckType.FILE)
    expect(proofer.failed_checks.length).toEqual(1)
  })
  ...
  it('allows for at-sign attribute', async () => {
    ...
    const proofer = await run_proofer(file, CheckType.FILE, {allow_hash_href: false})
    expect(proofer.failed_checks[0].description).toMatch(/linking to internal hash/)
  })
  ...
  it('works for a direct link through directory', async () => {
    ...
    const proofer = await run_proofer(dir, CheckType.DIRECTORY)
    expect(proofer.failed_checks).toEqual([])
  })
  ...
  it('reports linked internal through directory', async () => {
    ...
    const proofer = await run_proofer(file, CheckType.DIRECTORY)
    expect(proofer.failed_checks[0].description).toMatch(/the file exists, but the hash 'generating-and-submitting' does not/)
  })

  it('works for hash hrefs', async () => {
    const file = path.join(FIXTURES_DIR, 'links', 'hash/inner.html')
    const proofer = await run_proofer(file, CheckType.FILE, {allow_hash_href: true})
    expect(proofer.failed_checks).toEqual([])
  })
  ...
  it('should not try reading PDFs', async () => {
    ...
    const proofer = await run_proofer(file, CheckType.FILE)
    expect(proofer.failed_checks.length).toEqual(3)
    expect(proofer.failed_checks[0].description).toMatch(/internally linking to exists.pdf#page=2; the file exists, but the hash 'page=2' does not/)
    expect(last(proofer.failed_checks).description).toMatch(/internally linking to missing.pdf#page=2, which does not exist/)
  })
