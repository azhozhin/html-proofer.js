...
describe('Scripts test', () => {
  ...
  it('fails for missing internal src', async () => {
    ...
    const proofer = await createAndRunProofer(file, CheckType.FILE)
    expect(proofer.failedChecks.length).toEqual(1)
    expect(proofer.failedChecks[0].description).toMatch(/internal script reference doesnotexist.js does not exist/)
  })
  ...
  it('fails for absent content', async () => {
    ...
    const proofer = await createAndRunProofer(file, CheckType.FILE)
    expect(proofer.failedChecks[0].description).toMatch(/script is empty and has no src attribute/)
  })
