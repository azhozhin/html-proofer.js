...
describe('Scripts test', () => {
  ...
  it('works for broken script within pre', async () => {
    ...
    const proofer = await run_proofer(script_pre, CheckType.FILE)
    expect(proofer.failed_checks).toEqual([])
  })
  ...
  it('ignores links via ignore_urls', async () => {
    ...
    const proofer = await run_proofer(ignorable_links, CheckType.FILE, {ignore_urls: [/\/assets\/.*(js|css|png|svg)/]})
    expect(proofer.failed_checks).toEqual([])
  })
  ...
  it('translates src via swap_urls', async () => {
    ...
    const proofer = await run_proofer(file, CheckType.FILE, opts)
    expect(proofer.failed_checks).toEqual([])
  })
